...
axis[0].set_ylabel('Cumulative Mass (solar masses)')
...
axis[1].set_ylabel('Culumative Mass (solar masses)')
...
angular_momentum = np.zeros(len(radius_array))
def angularmomentum(distance, velocity, mass):
    for i in range(len(distance)):
        angular_momentum[i] += velocity[i]*distance[i]*mass[i]
    return angular_momentum

#preliminary plots have the angular momentum plotted against the distance from
#the center since this is something both galaxies would have in common
#This isn't firm though and I intend to talk in class tomorrow about what the 
#best plot would be

MWdisk_L = angularmomentum(radius_array, MW_velocity_disk, MW_mass_disk)
plt.plot(radius_array, MWdisk_L, label = 'MW disk L')

angular_momentum=np.zeros(len(radius_array))
M31_disk_L = angularmomentum(radius_array, M31_velocity_disk, M31_mass_disk)
plt.plot(radius_array, M31_disk_L, label = 'M31 disk L')
plt.legend()
plt.savefig('Merger_preliminary_plot_disk.png')
plt.figure()

angular_momentum = np.zeros(len(radius_array))
MWbulge_L = angularmomentum(radius_array, MW_velocity_bulge, MW_mass_bulge)
plt.plot(radius_array, MWbulge_L, label = 'MW bulge L')

angular_momentum = np.zeros(len(radius_array))
M31_bulge_L = angularmomentum(radius_array, M31_velocity_bulge, M31_mass_bulge)
plt.plot(radius_array, M31_bulge_L, label = 'M31 bulge L')
plt.legend()
plt.savefig('Merger_preliminary_plot_bulge.png')
