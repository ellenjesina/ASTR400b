...
def OrbitCOM(galaxyname, start, end, n):
    ...
    fileout = "Orbit_"+str(galaxyname)+"Merge_Bulge"+'.txt'
    ...
    for i, snap_id in enumerate(snaps_ids): # loop over files
        ...
        # Initialize an instance of CenterOfMass class, using disk or bulge particles
        CenterofMass = CenterOfMass(filename, 3)
...
#OrbitCOM('MW', 445, 800, 5)
#OrbitCOM('M31', 445, 800, 5)
...
MWdata = np.genfromtxt("Orbit_MWMerge.txt", dtype = None, names = True, skip_header = 0)
M31data = np.genfromtxt("Orbit_M31Merge.txt", dtype = None, names = True, skip_header = 0)
